...
public final class TFCCalendar implements INBTSerializable<NBTTagCompound> {
    public static final TFCCalendar INSTANCE = new TFCCalendar();
     ...
     */
    public static final ICalendar PLAYER_TIME = () -> TFCCalendar.INSTANCE.playerTime;
    ...
    public static final ICalendarFormatted CALENDAR_TIME = new ICalendarFormatted() {
        ...
        public long getTicks() {
            return TFCCalendar.INSTANCE.calendarTime;
        }

        @Override
        public long getDaysInMonth() {
            return TFCCalendar.INSTANCE.daysInMonth;
        }
    };
    ...
    public TFCCalendar() {
        ...
        daysInMonth = TFCConfig.General.MISC.defaultMonthLength;
        playerTime = 0;
    ...
    public long setTimeFromWorldTime(long worldTimeToSetTo) {
        ...
        long worldTimeJump = (worldTimeToSetTo % ICalendar.TICKS_IN_DAY) - TFCCalendar.CALENDAR_TIME.getWorldTime();
        if (worldTimeJump < 0) {
    ...
    public void resetTo(TFCCalendar resetTo) {
    ...
    public void setMonthLength(int newMonthLength) {
        ...
        long baseMonths = TFCCalendar.CALENDAR_TIME.getTotalMonths();
        long baseDayTime = calendarTime - (TFCCalendar.CALENDAR_TIME.getTotalDays() * ICalendar.TICKS_IN_DAY);
        // Minus one here because `getDayOfMonth` returns the player visible one (which adds one)
        float monthPercent = (float) (TFCCalendar.CALENDAR_TIME.getDayOfMonth() - 1) / daysInMonth;
