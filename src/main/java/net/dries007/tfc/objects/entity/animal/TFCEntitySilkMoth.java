...
import net.dries007.tfc.types.DefaultWoods;
...
public class TFCEntitySilkMoth extends TFCEntityAnimal implements ILivestock {
    ...
    public TFCEntitySilkMoth(World worldIn) {
        this(worldIn, Gender.valueOf(Constants.RNG.nextBoolean()), TFCEntityAnimal.getRandomGrowth(DAYS_TO_ADULTHOOD, DAYS_TO_ELDER));
    }

    public TFCEntitySilkMoth(World worldIn, Gender gender, int birthDay) {
    ...
    public boolean getCanSpawnHere() {
        return this.world.checkNoEntityCollision(getEntityBoundingBox())
                ...
                && (this.world.getBlockState(this.getPosition().down()) == TFCBlockLeaves.get(TFCRegistries.TREES.getValue(DefaultWoods.MULBERRY)));
    }
        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {
                for (int k = 0; k < 3; k++) {
                    ...
                    if (stateBlock == TFCBlockLeaves.get(TFCRegistries.TREES.getValue(DefaultWoods.MULBERRY))) {
