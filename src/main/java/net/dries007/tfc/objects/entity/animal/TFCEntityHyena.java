...
public class TFCEntityHyena extends TFCEntityAnimalMammal implements IPredator {
    private static final int DAYS_TO_ADULTHOOD = 112;

    @SuppressWarnings("unused")
    public TFCEntityHyena(World worldIn) {
        this(worldIn, Gender.valueOf(Constants.RNG.nextBoolean()),
                getRandomGrowth(DAYS_TO_ADULTHOOD, 0));
    }

    public TFCEntityHyena(World worldIn, Gender gender, int birthDay) {
    ...
    public int getSpawnWeight(Biome biome, float temperature, float rainfall, float floraDensity, float floraDiversity) {
        ...
        if (!TFCBiomes.isOceanicBiome(biome) && !TFCBiomes.isBeachBiome(biome) &&
            ...
            return TFCConfig.Animals.HYENA.rarity;
        }
        return 0;
    }
    ...
    protected void initEntityAI() {
        ...
        int priority = 2;
        for (String input : TFCConfig.Animals.HYENA.huntCreatures) {
    ...
    protected ResourceLocation getLootTable() {
        return TFCLootTables.ANIMALS_HYENA;
    }
