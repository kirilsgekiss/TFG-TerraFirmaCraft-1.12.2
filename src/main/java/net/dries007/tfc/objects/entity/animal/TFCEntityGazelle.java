...
public class TFCEntityGazelle extends TFCEntityAnimalMammal implements IHuntable {
    private static final int DAYS_TO_ADULTHOOD = 128;

    @SuppressWarnings("unused")
    public TFCEntityGazelle(World worldIn) {
        this(worldIn, Gender.valueOf(Constants.RNG.nextBoolean()), getRandomGrowth(DAYS_TO_ADULTHOOD, 0));
    }

    public TFCEntityGazelle(World worldIn, Gender gender, int birthDay) {
    ...
    public int getSpawnWeight(Biome biome, float temperature, float rainfall, float floraDensity, float floraDiversity) {
        ...
        if (!TFCBiomes.isOceanicBiome(biome) && !TFCBiomes.isBeachBiome(biome) &&
            ...
            return TFCConfig.Animals.GAZELLE.rarity;
        }
        return 0;
    }
    ...
    protected ResourceLocation getLootTable() {
        return TFCLootTables.ANIMALS_GAZELLE;
    }
