...
public class TFCAnimalFood {
    private static final HashMap<Class<? extends Entity>, TFCAnimalFood> ANIMAL_FOOD_MAP = new HashMap<>();

    @Nullable
    public static TFCAnimalFood get(Class<? extends Entity> animalClass) {
    ...
    public static void readFile(Set<Map.Entry<String, JsonElement>> jsonElements) {
        for (Map.Entry<String, JsonElement> entry : jsonElements) {
            try {
                ...
                TFCAnimalFood animalFood = GSON.fromJson(entry.getValue(), TFCAnimalFood.class);
    ...
    public TFCAnimalFood(boolean eatRotten) {
