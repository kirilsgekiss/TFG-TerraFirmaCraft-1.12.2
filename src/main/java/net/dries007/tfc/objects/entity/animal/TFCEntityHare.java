...
public class TFCEntityHare extends TFCEntityRabbit implements IHuntable {
    private static final int DAYS_TO_ADULTHOOD = 16;
    private static final DataParameter<Integer> HARE_TYPE = EntityDataManager.createKey(TFCEntityHare.class, DataSerializers.VARINT);

    @SuppressWarnings("unused")
    public TFCEntityHare(World worldIn) {
        this(worldIn, Gender.valueOf(Constants.RNG.nextBoolean()), getRandomGrowth(DAYS_TO_ADULTHOOD, 0));
    }

    public TFCEntityHare(World worldIn, Gender gender, int birthDay) {
        super(worldIn, gender, birthDay);
        this.setSize(0.4F, 0.6F);
        this.jumpHelper = new RabbitJumpHelper(this);
        this.moveHelper = new TFCEntityHare.RabbitMoveHelper(this);
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (livingdata instanceof TFCEntityHare.HareTypeData) {
            i = ((TFCEntityHare.HareTypeData) livingdata).typeData;
        } else {
            livingdata = new TFCEntityHare.HareTypeData(i);
        }
