...
public class TFCBlockFruitTreeLeaves extends BlockLeaves implements IGrowingPlant {
    ...
    public void randomTick(World world, BlockPos pos, IBlockState state, Random random) {
        if (!world.isRemote) {
            if (state.getValue(HARVESTABLE) && tree.isHarvestMonth(TFCCalendar.CALENDAR_TIME.getMonthOfYear())) {
                TETickCounter te = Helpers.getTE(world, pos, TETickCounter.class);
                if (te != null) {
                    long hours = te.getTicksSinceUpdate() / ICalendar.TICKS_IN_HOUR;
                    if (hours > (tree.getGrowthTime() * TFCConfig.General.FOOD.fruitTreeGrowthTimeModifier)) {
                        world.setBlockState(pos, state.withProperty(LEAF_STATE, EnumLeafState.FRUIT));
                        te.resetCounter();
                    }
                }
            } else if (tree.isFlowerMonth(TFCCalendar.CALENDAR_TIME.getMonthOfYear())) {
    ...
    public GrowthStatus getGrowingStatus(IBlockState state, World world, BlockPos pos) {
        ...
        } else if (!state.getValue(HARVESTABLE) && tree.isHarvestMonth(TFCCalendar.CALENDAR_TIME.getMonthOfYear())) {
