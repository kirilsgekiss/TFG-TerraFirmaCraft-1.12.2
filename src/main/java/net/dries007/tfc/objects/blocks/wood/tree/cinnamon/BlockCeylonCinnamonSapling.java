package net.dries007.tfc.objects.blocks.wood.tree.cinnamon;
...
public class BlockCeylonCinnamonSapling extends BlockSaplingTFC implements IGrowingPlant
{
    ...
    public void grow(World world, Random rand, BlockPos pos, IBlockState blockState)
    {
        ...
        IBlockState leaves = BlocksTFC.CEYLON_CINNAMON_LEAVES.getDefaultState();
        for (int trunk = 0; trunk < height; trunk++)
        {
            BlockPos trunkPos = pos.offset(EnumFacing.UP, trunk);
            world.setBlockState(trunkPos, BlocksTFC.CEYLON_CINNAMON_LOG.getDefaultState());
            if (trunk < 3)
                continue;
