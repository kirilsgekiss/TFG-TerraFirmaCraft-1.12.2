...
public class BlockSupportTFC extends Block
{
    ...
    private static final Map<Tree, BlockSupportTFC> MAP = new HashMap<>();

    public static BlockSupportTFC get(Tree wood)
    {
        return MAP.get(wood);
    }
    ...
    public static boolean isBeingSupported(World worldIn, BlockPos pos)
    {
            ...
            pos.add(sRangeHorNeg, sRangeVertNeg, sRangeHorNeg), pos.add(sRangeHor, sRangeVert, sRangeHor)))
        {
            ...
            if (st.getBlock() instanceof BlockSupportTFC)
            {
                if (((BlockSupportTFC) st.getBlock()).canSupportBlocks(worldIn, searchSupport))
                {
    ...
    public static Set<BlockPos> getAllUnsupportedBlocksIn(World worldIn, BlockPos from, BlockPos to)
    {
            ...
            maxPoint.add(sRangeHor, sRangeVert, sRangeHor)))
        {
            ...
            if (st.getBlock() instanceof BlockSupportTFC)
            {
                if (((BlockSupportTFC) st.getBlock()).canSupportBlocks(worldIn, searchingPoint))
                {
    ...
    public BlockSupportTFC(Tree wood)
    {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if (player.isSneaking() && heldStack.getItem() instanceof ItemBlock)
        {
            ...
            if (itemBlock instanceof BlockSupportTFC)
            {
    ...
    private boolean isConnectable(IBlockAccess world, BlockPos pos, EnumFacing facing)
    {
        ...
        return state.getBlock() instanceof BlockSupportTFC;
    }
    ...
    private boolean isThreeTall(IBlockAccess world, BlockPos pos)
    {
        ...
        if (!(world.getBlockState(pos.down()).getBlock() instanceof BlockSupportTFC)) return false;
        return world.getBlockState(pos.down().down()).getBlock() instanceof BlockSupportTFC;
    }
    ...
    {
        IBlockState state = world.getBlockState(pos);
        if (!(state.getBlock() instanceof BlockSupportTFC)) return false;
        EnumFacing.Axis axis = state.getValue(AXIS);
        if (axis == EnumFacing.Axis.Y)
        {
            return world.getBlockState(pos.down()).isNormalCube() || isConnectable(world, pos, EnumFacing.DOWN);
    ...
    private int getHorizontalDistance(EnumFacing face, IBlockAccess worldIn, BlockPos pos)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (!(worldIn.getBlockState(offsetPos).getBlock() instanceof BlockSupportTFC) && !worldIn.isAirBlock(offsetPos))
            {
                return 0;
            }
            IBlockState state = worldIn.getBlockState(pos.offset(face, i + 1));
            if (state.getBlock() instanceof BlockSupportTFC && state.getValue(AXIS) == EnumFacing.Axis.Y)
            {
                distance = i;
                break;
            }
