...
public class TFCBlockBerryBush extends Block implements IGrowingPlant {
    ...
    public void randomTick(World world, BlockPos pos, IBlockState state, Random random) {
        if (!world.isRemote) {
            ...
            if (te != null) {
                ...
                if (hours > (bush.getGrowthTime() * TFCConfig.General.FOOD.berryBushGrowthTimeModifier) && bush.isValidForGrowth(temp, rainfall)) {
                    if (bush.isHarvestMonth(TFCCalendar.CALENDAR_TIME.getMonthOfYear())) {
                        //Fruiting
    ...
    public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
        if (!(entityIn instanceof EntityPlayer && ((EntityPlayer) entityIn).isCreative())) {
            ...
            entityIn.motionX *= TFCConfig.General.MISC.berryBushMovementModifier;
            if (entityIn.motionY < 0) {
                entityIn.motionY *= TFCConfig.General.MISC.berryBushMovementModifier;
            }
            entityIn.motionZ *= TFCConfig.General.MISC.berryBushMovementModifier;
            if (bush.isSpiky() && entityIn instanceof EntityLivingBase) {
                entityIn.attackEntityFrom(TFCDamageSources.BERRYBUSH, 1.0F);
            }
        }
    }
    ...
    public GrowthStatus getGrowingStatus(IBlockState state, World world, BlockPos pos) {
        ...
        } else if (canGrow && bush.isHarvestMonth(TFCCalendar.CALENDAR_TIME.getMonthOfYear())) {
