...
public class TFCBlockSaguaroCactus extends TFCBlockPlant implements IGrowable, ITallPlant
{
    ...
    private static final Map<Plant, TFCBlockSaguaroCactus> MAP = new HashMap<>();
    
    public static final AxisAlignedBB CACTUS_ROOT = new AxisAlignedBB(0.25D, 0.0D, 0.25D, 0.75D, 1.0D, 0.75D);

    public static TFCBlockSaguaroCactus get(Plant plant)
    {
        return TFCBlockSaguaroCactus.MAP.get(plant);
    }

    public TFCBlockSaguaroCactus(Plant plant)
    {
        super(plant);
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        if (state.getBlock() instanceof TFCBlockSaguaroCactus && tile.isSet()) state = state.getBlock().getActualState(state, worldIn, pos);

        if (state.getBlock() instanceof TFCBlockSaguaroCactus && state.getValue(HORIZONTAL))
        {
            IBlockState stateHorizontal = worldIn.getBlockState(pos.offset(state.getValue(HORIZONTAL_DIRECTION)));

            if(stateHorizontal.getBlock() instanceof TFCBlockSaguaroCactus)
            {
