...
public class TFCBlockTorch extends BlockTorch implements IItemSize, ILightableBlock {
    public static boolean canLight(ItemStack stack) {
        return stack.getItem() == Item.getItemFromBlock(Blocks.TORCH) || TFCItemFireStarter.canIgnite(stack);
    }

    public TFCBlockTorch() {
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        ...
        if (te != null) {
            if (!worldIn.isRemote && te.getTicksSinceUpdate() > TFCConfig.General.OVERRIDES.torchTime && TFCConfig.General.OVERRIDES.torchTime > 0) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            ...
            if (state.getValue(LIT)) {
                ...
                if (TFCBlockTorch.canLight(stack)) {
