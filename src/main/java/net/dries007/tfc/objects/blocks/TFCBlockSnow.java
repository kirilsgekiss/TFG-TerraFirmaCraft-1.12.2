...
public class TFCBlockSnow extends BlockSnow implements ITemperatureBlock {
    public TFCBlockSnow() {
        setHardness(0.1F);
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        ...
        if (worldIn.getLightFor(EnumSkyBlock.BLOCK, pos) > 11 - getLightOpacity(state, worldIn, pos) || TFCClimate.getActualTemp(worldIn, pos) > 4f) {
    ...
    public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) {
        if (!(entityIn instanceof EntityPlayer && ((EntityPlayer) entityIn).isCreative())) {
            entityIn.motionX *= TFCConfig.General.MISC.snowMovementModifier;
            entityIn.motionZ *= TFCConfig.General.MISC.snowMovementModifier;
        }
    }
    ...
    public void onTemperatureUpdateTick(World world, BlockPos pos, IBlockState state) {
        if (world.isRaining() && world.getLightFor(EnumSkyBlock.BLOCK, pos.up()) < 11 - getLightOpacity(state, world, pos)) {
            int expectedLayers = -2 - (int) (TFCClimate.getActualTemp(world, pos) * 0.5f);
