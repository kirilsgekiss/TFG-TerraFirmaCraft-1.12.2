...
public class TFCBlockIce extends BlockIce implements ITemperatureBlock {
    private final Fluid waterFluid;
    private final float meltThreshold;

    public TFCBlockIce(Fluid waterFluid) {
        this.waterFluid = waterFluid;
        this.meltThreshold = waterFluid == TFCFluids.SEA_WATER.get() ? IceMeltHandler.SALT_WATER_MELT_THRESHOLD : IceMeltHandler.ICE_MELT_THRESHOLD;
    ...
    public void onTemperatureUpdateTick(World world, BlockPos pos, IBlockState state) {
        ...
        if (world.getLightFor(EnumSkyBlock.BLOCK, pos) > 11 - getLightOpacity(state, world, pos) || TFCClimate.getActualTemp(world, pos) > meltThreshold) {
