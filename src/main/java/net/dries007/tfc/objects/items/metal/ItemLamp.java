...
public class ItemLamp extends TFCItem implements IMetalItem {

    private final Material material;

    private static final Map<Material, ItemLamp> MAP = new HashMap<>();
    public static ItemLamp get(gregtech.api.unification.material.Material metal) {
        return MAP.get(metal);
    }

    public ItemLamp(Material material)
    {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        if (facing != null)
        {
                    ...
                    stateSupport.isSideSolid(worldIn, supportPos, EnumFacing.UP)) // forge says to do it this way, IBlockProperties#isTopSolid
            {
                if (!worldIn.isRemote)
                {
                    ItemLamp lamp = (ItemLamp) stack.getItem();
                    worldIn.setBlockState(placedPos, TFCBlockLamp.get(lamp.material).getDefaultState());
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        return LocalizationUtils.format("item.tfc.metal.lamp.name", material.getLocalizedName());
    }
