        ...
        if (metal != null)
        {
            ...
            switch (ConfigTFC.Client.TOOLTIP.oreTooltipMode)
            {
                ...
                case UNIT_ONLY:
                    ...
                    String info = String.format("%s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units", smeltAmount));
                    tooltip.add(info);
                    break;
                case TOTAL_ONLY:
                    // not like Classic, "Metal: xx total units" Adds the whole stacks worth up.
                    String stackTotal = String.format("%s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units.total", smeltAmount * stack.getCount()));
                ...
                case ALL_INFO:
                    ...
                    if (stack.getCount() > 1)
                    {
                        infoTotal = String.format("%s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units.info_total", smeltAmount, smeltAmount * stack.getCount()));
                    }
                    else
                    {
                        infoTotal = String.format("%s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units", smeltAmount));
                    }
                ...
                case ADVANCED:
                    ...
                    if (stack.getCount() > 1)
                    {
                        advancedTotal = String.format("%s: %s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units.info_total", smeltAmount, smeltAmount * stack.getCount()), I18n.format("tfc.tooltip.melttemp", (int) metal.getFluid().getTemperature()));
                    }
                    else
                    {
                        advancedTotal = String.format("%s: %s", I18n.format(metal.getUnlocalizedName()), I18n.format("tfc.tooltip.units", smeltAmount));
                    }
