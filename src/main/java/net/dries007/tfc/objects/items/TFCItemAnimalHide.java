...
public class TFCItemAnimalHide extends TFCItem {
    private static final Map<HideType, Map<HideSize, TFCItemAnimalHide>> TABLE = new HashMap<>();

    @Nonnull
    public static TFCItemAnimalHide get(HideType type, HideSize size) {
    ...
    public TFCItemAnimalHide(HideType type, HideSize size) {
        this.type = type;
        this.size = size;
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (TFCConfig.General.OVERRIDES.enableThatchBed && type == HideType.RAW && size == HideSize.LARGE && facing == EnumFacing.UP && worldIn.getBlockState(pos).getBlock() == TFCBlocks.THATCH && worldIn.getBlockState(pos.offset(player.getHorizontalFacing())).getBlock() == TFCBlocks.THATCH) {
    ...
    public ItemStack getContainerItem(ItemStack itemStack) {
        switch (size) {
            case SMALL:
                return new ItemStack(TFCItemAnimalHide.get(HideType.RAW, HideSize.SMALL));
            case MEDIUM:
                return new ItemStack(TFCItemAnimalHide.get(HideType.RAW, HideSize.MEDIUM));
            case LARGE:
                return new ItemStack(TFCItemAnimalHide.get(HideType.RAW, HideSize.LARGE));
        }
        return new ItemStack(TFCItemAnimalHide.get(HideType.RAW, HideSize.SMALL));
    }
