...
public final class TFCFluids {
    ...
    public static ImmutableCollection<FluidWrapper> getAllMaterialFluids() {
        return allMaterialFluids.values();
    }
    ...
    public static FluidWrapper getWrapper(@Nonnull Fluid fluid) {
        if (!WRAPPERS.containsKey(fluid)) {
            // Should only ever get called for non-tfc fluids, but in which case prevents a
            // null wrapper getting returned
        ...
        // Water variants
        {
            ...
            SEA_WATER = registerFluid(new Fluid("sea_water", STILL, FLOW, 0xFF1F5099)).with(DrinkableProperty.DRINKABLE, player -> {
                if (player.getFoodStats() instanceof FoodStatsTFC) {
                    ...
                    if (Constants.RNG.nextDouble() < TFCConfig.General.PLAYER.chanceThirstOnSaltyDrink) {
                        player.addPotionEffect(new PotionEffect(TFCPotionEffects.THIRST, 600, 0));
                    }
                }
            });
        }
        ...
        // Alcohols
        {
            DrinkableProperty alcoholProperty = player -> {
                ...
                if (player.getFoodStats() instanceof FoodStatsTFC && playerData != null) {
                    ...
                    if (playerData.getIntoxicatedTime() > 24 * ICalendar.TICKS_IN_HOUR && Constants.RNG.nextFloat() < 0.25f) {
