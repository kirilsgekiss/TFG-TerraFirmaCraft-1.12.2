...
public class TETFCAnvil extends TEInventory {
    ...
    public boolean isStone() {
        ...
        return state.getBlock() instanceof TFCBlockRockAnvil;
    }

    public int getTier() {
        IBlockState state = world.getBlockState(pos);
        if (state.getBlock() instanceof TFCBlockMetalAnvil) {
            return ((TFCBlockMetalAnvil) state.getBlock()).getMetal().getProperty(TFCPropertyKey.TFC).getMaterialTier();
        }
        return 0;
    }
    ...
    public boolean isItemValid(int slot, ItemStack stack) {
        switch (slot) {
            ...
            case SLOT_HAMMER:
                return OreDictionaryHelper.doesStackMatchOre(stack, "craftingToolHammer");
            default:
                return false;
        }
    }
    ...
    public void addStep(@Nonnull EntityPlayer player, @Nullable ForgeStep step) {
        ...
        if (cap != null) {
            ...
            if (recipe != null) {
                ...
                int workMin = workingTarget - TFCConfig.General.DIFFICULTY.acceptableAnvilRange;
                int workMax = workingTarget + TFCConfig.General.DIFFICULTY.acceptableAnvilRange;
