...
public class TEBloomery extends TETickableInventory implements ITickable {
    ...
    public long getRemainingTicks() {
        return TFCConfig.Devices.BLOOMERY.ticks - (TFCCalendar.PLAYER_TIME.getTicks() - litTick);
    }
    ...
    public void onIgnite() {
        this.litTick = TFCCalendar.PLAYER_TIME.getTicks();
    }
    ...
    protected boolean isInternalBlockComplete() {
        ...
        return inside.getBlock() == TFCBlocks.CHARCOAL_PILE && inside.getValue(TFCBlockCharcoalPile.LAYERS) >= 8;
    }
    ...
    protected void updateSlagBlock(boolean cooking) {
        ...
        for (int i = 1; i < 4; i++) {
            if (slagLayers > 0) {
                if (slagLayers >= 4) {
                    slagLayers -= 4;
                    world.setBlockState(getInternalBlock().up(i), TFCBlocks.MOLTEN.getDefaultState().withProperty(LIT, cooking).withProperty(TFCBlockMolten.LAYERS, 4));
                } else {
                    world.setBlockState(getInternalBlock().up(i), TFCBlocks.MOLTEN.getDefaultState().withProperty(LIT, cooking).withProperty(TFCBlockMolten.LAYERS, slagLayers));
                    slagLayers = 0;
                }
            } else {
